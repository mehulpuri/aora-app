...
export const saveVideoForUser = async (userId, videoId) => {
  try {
    ...
    const updatedSavedVideos = userDoc.savedVideos || [];
    if (!updatedSavedVideos.includes(videoId)) {
...
export const getSavedUserPosts = async (userId) => {
  try {
    ...
    // Extract the savedVideos array from the user document
    const { savedVideos } = user;

    if (!savedVideos || savedVideos.length === 0) {
      // Return an empty array if no videos are saved
      return [];
    }

    // Function to fetch posts for a single video ID
    const fetchPostsForVideoId = async (videoId) => {
      return databases.listDocuments(
        config.databaseId,
        config.videoCollectionId,
        [Query.equal("$id", videoId)] // Query for posts where $id matches the given videoId
      );
    };

    // Fetch posts for each saved video ID and combine results
    const fetchAllPosts = async () => {
      const allPosts = [];
      for (const videoId of savedVideos) {
        const { documents } = await fetchPostsForVideoId(videoId);
        allPosts.push(...documents); // Add documents from each query to the allPosts array
      }
      return allPosts;
    };
